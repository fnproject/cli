...
func TestCodeOnlyBuild(t *testing.T) {
	...
	buildAppName := buildHarness.NewAppName()
	buildHarness.Fn("create", "app", buildAppName).AssertSuccess()
...
`, common.LatestYamlVersion), 0644)
		...
		h.Fn("build", "--app", buildAppName).AssertFailed().AssertStderrContains("native/ is not allowed for single-architecture Python functions")
	})
...
`, common.LatestYamlVersion), 0644)
		...
		h.Fn("build", "--app", buildAppName).AssertFailed().AssertStderrContains("native/ is not allowed for single-architecture Node.js functions")
	})
...
`, common.LatestYamlVersion), 0644)
		...
		h.Fn("build", "--app", buildAppName).AssertFailed().AssertStderrContains("Maven was not found in PATH")
	})
}
