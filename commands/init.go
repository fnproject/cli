...
func runtimeToolRequirement(runtime string) (string, string, []string) {
	...
	case strings.HasPrefix(baseRuntime, "node") || strings.HasPrefix(baseRuntime, "javascript"):
		return runtimeDisplayName(runtime), "Node.js", []string{"node"}
	default:
		return "", "", nil
	}
}
...
func baseRuntimeFromName(runtimeName string) string {
	...
	if lower == "ol9" || lower == "ol8" {
		// Managed Go runtimes may be exposed as generic OS runtime names.
		return "go"
	}
...
func createPythonCodeOnlyBoilerplate(path string) (bool, error) {
	functionDir := filepath.Join(path, "function")
	if err := os.MkdirAll(functionDir, 0755); err != nil {
		return false, err
	}
	filePath := filepath.Join(functionDir, "hello_world.py")
...
func createNodeCodeOnlyBoilerplate(path string) (bool, error) {
	...
	if _, err := os.Stat(packageJSONPath); os.IsNotExist(err) {
		pkg := "{\n" +
			...
			"  }\n" +
			"}\n"
		if err := os.WriteFile(packageJSONPath, []byte(pkg), 0644); err != nil {
			return false, err
		}
	} else if err != nil {
		return false, err
	...
	filePath := filepath.Join(functionDir, "func.js")
	if _, err := os.Stat(filePath); err == nil {
		return false, nil
	} else if !os.IsNotExist(err) {
		return false, err
