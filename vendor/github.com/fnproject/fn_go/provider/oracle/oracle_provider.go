...
func signRequest(signer common.HTTPRequestSigner, request *http.Request) (signedRequest *http.Request, err error) {
	...
	// workaround for DP Hostname spoofing of requests
	if request.Host != "" {
		// since signer.Sign uses request.URL.Host to verify a request,
		// we need to copy the value over used for the fake DNS workaround
		// set in request.Host
		originalHost := request.URL.Host
		request.URL.Host = request.Host
		err = signer.Sign(request)
		// reset the request URL host to the original before making the request
		request.URL.Host = originalHost

	} else { // CP requests don't need to spoof DNS
		err = signer.Sign(request)
	}
